...
for verb in verbs.lemme:
    ...
    r = requests.get(url_base_DES_online + verb)
    ...
    if r.status_code == 200:
        ...
        for link in soup.find_all("a"):
            if "/des/synonymes/" in link.get("href"):
                ...
                if word != verb and word not in synonymes:
        ...
        for j, synonyme in enumerate(synonymes_ord):
            ...
            verbs.loc[verbs.lemme == verb, col_word] = synonymes_ord[j]
            num_syn += 1

    else:
        verbs_not_found.append(verb)
        print(f"The word {verb} has not been found in the DES")
        continue
    ...
    if count > 200:
        ...
        count = 0
        verbs.to_csv("data/lemmes_verbes_avec_syn.csv")
...
##############################################################################
with open("data/verbs_not_found_syn.csv", 'w', newline='', encoding='utf8') as csvfile:
    writer = csv.writer(csvfile)

    writer.writerow(['verbs_not_found'])
    for n in verbs_not_found:
        writer.writerow([n])
